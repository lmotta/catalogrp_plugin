...
from catalogeos_plugin import API_EOS

@qgsfunction(args=1, group='EOS')
def getValueFromMetadata(values, feature, parent):
  ...
  <p><h4>Example</h4>getValueFromMetadata( '"cloudCoverage"' )</p><p>Return: Percentage of cloud</p>
  """
  if values[0].count('"') % 2 != 0:
    raise Exception("Catalog EOS: Error! Key need double quotes: %s." % values[0] )
  
  if len( values[0] ) < 1:
    raise Exception("Catalog EOS: Error! Field is empty." )
    

  name_metadata_json = 'meta_json'
  id_metadata_json = feature.fieldNameIndex( name_metadata_json )
  if id_metadata_json == -1:
    raise Exception("Catalog EOS: Error! Need have '%s' field." % name_metadata_json )
